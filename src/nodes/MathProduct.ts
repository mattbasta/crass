...
export default class MathProduct implements Expression {
  ...
  async pretty(indent: number) {
    let output = '';
    const base = await this.base.pretty(indent);
    const term = await this.term.pretty(indent);
    output += this.base instanceof objects.MathSum ? '(' + base + ')' : base;
    output += ' ';
    output += this.operator;
    output += ' ';
  ...
  async optimize(kw: OptimizeKeywords) {
    ...
    if (
      ...
      const val = new objects.Number(
        this.base.asNumber() * this.term.asNumber(),
      );
      ...
      const val = new objects.Number(
        this.term.asNumber() * this.base.asNumber(),
      );
