...
function isDeclaration(item) {
    return item instanceof objects.Declaration;
}
...
function canRulesetsBeCombined(parentBody, xIdx, yIdx) {
    ...
    for (var i = yIdx; i > xIdx; i--) {
        if (!isDeclaration(parentBody[i])) continue;

        tempSelector = normalizeSelector(parentBody[i]);
