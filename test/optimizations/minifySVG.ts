import * as assert from 'assert';

import {optimized} from '../_helpers';

const origURI =
  'url(data:image/svg+xml;base64,)';
const optimizedURI =
  'url(data:image/svg+xml;base64,)';

const regression1 = `
.has-svg:before {
    content: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='100%' height='100%' viewBox='-0.5 0 20 15'><rect fill='white' stroke='none' transform='rotate(45 4.0033 8.87436)' height='5' width='6.32304' y='6.37436' x='0.84178'></rect><rect fill='white' stroke='none' transform='rotate(45 11.1776 7.7066)' width='5' height='16.79756' y='-0.69218' x='8.67764'></rect></svg>");
}
`;

describe('minify SVG', () => {
  it('should not minify SVGs without O1', async () => {
    assert.equal(
      await optimized(`a{foo:${origURI}}`),
      `a{foo:${origURI}}`,
      'Should be unchanged',
    );
  });
  it('should minify SVGs with O1', async () => {
    assert.equal(
      await optimized(`a{foo:${origURI}}`, {o1: true}),
      `a{foo:${optimizedURI}}`,
      'Should be minified',
    );
  });
  it('should handle the regression test', async () => {
    assert.ok(await optimized(regression1, {o1: true}));
  });
});
