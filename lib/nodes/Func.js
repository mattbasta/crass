...
Func.prototype.optimize = function optimize(kw) {
    ...
    self = self.optimizeRadialGradient();
    if (!self) {
        kw.func = oldkwf;
        return null;
    }
...
Func.prototype.optimizeLinearGradient = function optimizeLinearGradient(kw) {
    ...
    segments.forEach((group, idx) => {
        if (group.length !== 2 || !(group[1][1] instanceof Dimension || group[1][1] instanceof Number)) {
            return;
        }
        ...
        if (!lastStop) {
            ...
            if (lastStop instanceof Number && lastStop.asNumber() !== 0) {
                return;
            }
            group[1][1] = null;
            return;
 ...
 * @return {Func}
 */
Func.prototype.optimizeRadialGradient = function optimizeRadialGradient() {
    if (
    ...
    this.content = new Expression(chain);
    return this;
};
