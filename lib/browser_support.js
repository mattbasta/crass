...
function match_browser(browserObj, kw) {
    var supportedBrowsers = Object.keys(browserObj).filter(function(key) {
        return key in kw.browser_min;
    });
    if (!supportedBrowsers.length) {
        return true;
    }
    return supportedBrowsers.some(function(key) {
        return kw.browser_min[key] < browserObj[key];
    });
}
